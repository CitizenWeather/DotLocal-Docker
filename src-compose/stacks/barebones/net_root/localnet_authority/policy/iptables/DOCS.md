...
Applies a **default-deny ingress** host firewall at stack startup using nftables (preferred) or iptables (fallback).
Rules are applied once by a privileged one-shot container that runs with `NET_ADMIN` + `NET_RAW` capabilities in host
networking mode.
...
Inbound traffic from Docker bridge networks (backbone, default, NAT) is always allowed. ICMP is always allowed. All
...
The host kernel must have nftables (`nft`) or iptables available. The container installs both from Alpine apk at
startup.
